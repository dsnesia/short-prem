...
class Setup {
    ...
    private function step(Request $request){
        ...
        echo '<h1>Premium URL Shortener</h1> 
            '.message().'
            <p>Premium URL Shortener Requirements</p>
            <div class="content">
                <p>
                PHP Version (v7.4+)
                '.$this->verify('version').'
            ...
            <div class="content">
                ...
                <ul>
                    ...
                    <li>
                        <strong><i>public/content/</i> folder must be writable.</strong> '.$this->verify('content').'
                        <p class="info '.(!$this->error ? 'hide': '').'">
            ...
            if(!$this->error) echo '<p><a href="validate" class="button">Proceed</a></p>';
        $this->footer();
    }
    /**
     * Validate
    ...
    public function stepvalidate($request){
        ...
        echo '<h2>2.0 Validate Purchase Code</h2>
        '.message().'
        <p>
        ...
        <form method="post" action="" class="form" autocomplete="off">
            ...
            <input type="text" name="code" class="input" placeholder="e.g. 10101010-10aa-0101-0101-a1b010a01b10" value="10101010-10aa-0101-0101-a1b010a01b10">
    ...
    public function stepdatabase(Request $request){
        ...
        echo '<h2>3.0 Database Configuration</h2>
        '.message().'
        <p>
        ...
        if($request->isPost()){
            ...
            if($this->error || !empty($this->error)){
                ...
                try{
                    ...
                    $request->save('password', $request->pass);
                    $request->save('site', $request->url);
    ...
    private function stepfinal($request){
        ...
        echo''.message().'<p>
            ...
            <p><a href="'.old('site').'" class="button">Visit your site</a></p>';
        $this->footer();
    }
    /**
     * Verify
    ...
    private function header($request){
        ...
        <html lang="en">
            ...
            <div class="left">
                <h3>Installer</h3>
                <ol>
                    <li>Requirements '.($request->segment(1) ? "<span>Completed</span>":"").'</li>
                    <li>Validate Purchase Code '.(in_array($request->segment(1), ['database', 'user', 'final']) ? "<span>Completed</span>":"").'</li>
                    <li>Database Installation'.(in_array($request->segment(1), ['user', 'final'])?"<span>Completed</span>":"").'</li>
                    <li>Admin Account'.($request->segment(1) == 'final' ?"<span>Completed</span>":"").'</li>
