...
class Paddle {
    ...
    public static function settings(){
		...
		$html = '<div class="form-group">
					<label for="paddle[enabled]" class="form-label">'.e('Paddle Payments').'</label>
					...
					<p class="form-text">'.e('Collect payments securely with Paddle.').'</p>
				</div>
    ...
    public static function payment(Request $request, int $id, string $type){
        ...
        if(!$request->name || !$request->address || !$request->city || !$request->country || !$request->zip || !$request->state) {
            return Response::factory(['error' => true, 'message' => e('Please fill your billing information.'), 'token' => csrf_token()])->json();
        }
