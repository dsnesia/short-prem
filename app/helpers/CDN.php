...
final class CDN {
    ...
    public static function providers($type = null){
        $list = [
            ...
            'contabo' => [
                ...
                'host' => 'https://{bucket}.{region}.contabostorage.com'                
            ],
            'vultr' => [
                'name' => 'Vultr Objects',
            ...
            'wasabi' =>[
                'name' => 'Wasabi Storage',
                'host' => 'https://{bucket}.s3.{region}.wasabisys.com'
            ]
        ];
    ...
    public function httpSigned(string $http, string $key, array $params = []){
        ...
        if(!isset($headers['x-amz-content-sha256'])) $headers['x-amz-content-sha256'] = hash('sha256', null);

        foreach($params as $header => $value){
            if(isset($cdn['noheaders']) && in_array($header, $cdn['noheaders'])) continue;
