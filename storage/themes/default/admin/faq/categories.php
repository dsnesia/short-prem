<h1 class="h3 mb-5"><?php ee('Article Categories') ?></h1>
<div class="row">
    <div class="col-md-4">
        <div class="card">
            <div class="card-body">
                <form method="post" action="<?php echo route('admin.faq.categories.save') ?>" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-4">
                        <label for="icon" class="form-label"><?php ee('Icon') ?></label>
                    ...
                    <button type="submit" class="btn btn-primary"><i data-feather="plus"></i> <?php ee('Add Category') ?></button>
                </form>
            </div>
        </div>
    </div>
                ...
                <table class="table table-hover my-0">
                    ...
                    <tbody>
                        <?php foreach($categories as $id => $category): ?>
                            <tr>
                                <td><a href="<?php echo route('help.category', [$id]) ?>" target="_blank"><?php echo isset($category->icon) && $category->icon ? '<i class="'.$category->icon.' me-1"></i>' : '' ?><?php echo $category->title ?></a></td>
    ...
    <div class="modal-content">
        <form action="#" method="post">
            <div class="modal-header">
                <h5 class="modal-title"><?php ee('Edit Category') ?></h5>
            ...
            <div class="modal-body">
                ...
                <div class="form-group mb-4">
                    <label for="newicon" class="form-label"><?php ee('Icon') ?></label>
                    <input type="text" class="form-control p-2" name="icon" id="newicon" placeholder="Fontawesome Icon" autocomplete="off">
                </div>
                <div class="form-group mb-4">
                    <label for="newlang" class="form-label"><?php ee('Language') ?></label>
                    <input type="text" class="form-control p-2" name="newlang" id="newlang" placeholder="e.g. en" autocomplete="off">
                </div>
                <div class="form-group mb-4">
